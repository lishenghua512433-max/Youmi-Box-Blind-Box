...
```
├── contracts/                 # BSC智能合约
│   ├── YoumiPayoutPool.sol    # 兑付合约源码(回收+佣金出款)
│   └── YoumiPayoutPool.json   # 合约ABI+Bytecode(编译产物)
├── scripts/
...
│   ├── lib/
│   │   ├── blockchain.ts      # 服务端合约交互(兑付/验证/余额查询)
...
- 资金安全: 收款钱包与兑付合约分离, 合约余额不足暂停回收提现
- 合约兑付: YoumiPayoutPool合约负责回收兑付+佣金发放, 管理员钱包为唯一owner
- 资金流向: 盲盒全款→私人钱包, 交易手续费→私人钱包, 回收兑付→合约出款, 佣金→合约出款
